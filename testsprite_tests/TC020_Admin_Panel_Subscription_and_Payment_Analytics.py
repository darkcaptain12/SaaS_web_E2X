...
async def run_test():
    ...
    try:
        ...
        # -> Click on the 'Yönetim' (Management) menu or 'Yönetim Paneline Git' button to access the admin management panel for subscriptions and payments.
        frame = context.pages[-1]
        # Click the 'Yönetim Paneline Git' button to go to the admin management panel.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Subscription Payment Overview').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Admin was unable to view subscriptions and payment records with correct filters and summary analytics as per the test plan.")
